...
export function DateCell<TData>({
  ...
  const [timeValue, setTimeValue] = React.useState(() =>
    initialValue ? extractTime(initialValue) : "00:00",
  );
  const containerRef = React.useRef<HTMLDivElement>(null);
  ...
  if (initialValue !== prevInitialValueRef.current) {
    prevInitialValueRef.current = initialValue;
    setValue(initialValue ?? "");
    setTimeValue(initialValue ? extractTime(initialValue) : "00:00");
  }

  const selectedDate = value ? (parseLocalDate(value) ?? undefined) : undefined;

  const saveDateTime = React.useCallback(
    (date: Date, time: string) => {
      if (readOnly) return;
      const [hours, minutes] = time.split(":").map(Number);
      date.setHours(hours ?? 0, minutes ?? 0, 0, 0);
      const formatted = formatDateTimeToString(date);
      setValue(formatted);
  ...
  const onDateSelect = React.useCallback(
    (date: Date | undefined) => {
      ...
      saveDateTime(date, timeValue);
    },
    [saveDateTime, timeValue, readOnly],
  );
  ...
  const onTimeChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setTimeValue(time);
      if (selectedDate) {
        saveDateTime(new Date(selectedDate), time);
      }
    },
    [selectedDate, saveDateTime],
  );
