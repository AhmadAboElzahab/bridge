...
interface TableContainerProps {
  entityType: "Maid" | "User";
  model: "maids" | "users";
}

export function TableContainer({ entityType, model }: TableContainerProps) {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({
    queryKey: ["tabs", entityType],
    queryFn: () => fetchTabs(entityType),
  });
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["tabs", entityType] });

  const createMutation = useMutation({
    mutationFn: (name: string) => createTab(entityType, name),
    onSuccess: (result) => {
      void invalidate();
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              placeholder={`e.g. All ${entityLabel}`}
              onKeyDown={(e) => {
